...
如果图片生成经常遇到前端长连接断开、`HTTP 524` 或网关超时，可以在本地额外启动一层异步中转服务。前端提交后会立即拿到 `task_id`，再轮询任务结果，避免浏览器请求一直挂着等待。

```bash
ASYNC_IMAGE_UPSTREAM_URL=https://api.gavinteam.online/v1 npm run async:image-server
```

如果希望队列和任务状态落到 Redis，并让本地 worker 支持更高并发，也可以使用 Go 版异步服务：

```bash
redis-server
ASYNC_IMAGE_UPSTREAM_URL=https://api.gavinteam.online/v1 ASYNC_IMAGE_CONCURRENCY=3 npm run async:image-server:go
```
...
REDIS_PASSWORD=
REDIS_DB=0
ASYNC_IMAGE_CONCURRENCY=3
ASYNC_IMAGE_REQUEST_TIMEOUT_MS=900000
ASYNC_IMAGE_TASK_TTL_MS=3600000
ASYNC_IMAGE_FILE_TTL_MS=3600000
ASYNC_IMAGE_FILE_STORE_DIR=/tmp/gpt-image-playground-async-images
ASYNC_IMAGE_PUBLIC_BASE_URL=
ASYNC_IMAGE_UPSTREAM_STREAM=false
ASYNC_IMAGE_STREAM_FALLBACK=true
ASYNC_IMAGE_UPSTREAM_RETRY_ATTEMPTS=1
ASYNC_IMAGE_DIAL_TIMEOUT_MS=30000
ASYNC_IMAGE_TLS_HANDSHAKE_TIMEOUT_MS=30000
ASYNC_IMAGE_FORCE_RESPONSES=true
ASYNC_IMAGE_UPSTREAM_HTTP2=false
ASYNC_IMAGE_UPSTREAM_DISABLE_KEEPALIVE=true
```

Go 版默认使用“内部非流式请求、外部异步轮询”：前端仍然只看到 `task_id`，后台 worker 等上游完成后再保存任务结果。若你的上游流式更稳定，可设 `ASYNC_IMAGE_UPSTREAM_STREAM=true`，后台会读取上游 SSE 并在完成后转成普通 JSON 给前端轮询使用。

默认还会把 `/images/generations` 后台转发为 `/responses + image_generation`，再把结果转换回 Images API 形状给前端轮询使用。生成完成后，Go 服务会把返回的 base64 图片写入 `ASYNC_IMAGE_FILE_STORE_DIR`，任务结果里只保留 `data[].url`，Redis 不再保存大块图片 base64。图片文件默认按 `ASYNC_IMAGE_FILE_TTL_MS` 清理，默认与任务 TTL 一致，也就是 1 小时；部署到服务器后如果外部访问地址不是请求 Host，可用 `ASYNC_IMAGE_PUBLIC_BASE_URL` 指定公开地址。如果需要强制直连 Images API，可设 `ASYNC_IMAGE_FORCE_RESPONSES=false`。

当非流式上游请求遇到 `TLS handshake timeout`、`unexpected EOF`、`502/503/504/524` 这类可恢复错误时，Go worker 会关闭空闲连接并按配置重试。默认 `ASYNC_IMAGE_STREAM_FALLBACK=true`，所以非流式主请求失败后会自动改用一次内部流式请求兜底；前端仍然只使用异步轮询，不会直接持有长连接。`400/401` 这类明确参数或鉴权错误不会重试。

启动后可在设置页导入以下配置 URL：

```text
http://127.0.0.1:8789/provider-settings.json
```

也可以使用仓库内置的静态配置文件：

```text
http://127.0.0.1:5173/local-async-provider.json
```

> 注意：这层服务端能把“前端长请求”改成“提交任务 + 轮询”，但如果后台继续请求的上游域名本身被 Cloudflare/Caddy/Nginx 提前断开，任务仍会失败。要彻底解决上游 `524`，需要让这层服务端直连真实上游，或部署到中转站内部并由后台 worker 执行图片生成。
