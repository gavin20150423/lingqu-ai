...
vi.mock('./lib/openaiCompatibleImageApi', () => ({
  ...
  getOpenAICompatibleQueuedImageResult: vi.fn(async () => ({
    images: [],
    actualParams: {},
    actualParamsList: [],
    revisedPrompts: [],
...
describe('fal task recovery', () => {
  ...
  it('recovers an OpenAI-compatible async task after store init', async () => {
    const profile = createDefaultOpenAIProfile({
      id: 'openai-profile',
      apiKey: 'test-key',
    })
    const openAITask = task({
      id: 'openai-async-task',
      apiProvider: 'openai',
      apiProfileId: profile.id,
      apiProfileName: profile.name,
      apiModel: profile.model,
      apiMode: 'images',
      customTaskId: 'img-task-1',
      status: 'running',
      finishedAt: null,
      elapsed: null,
    })
    await putDbTask(openAITask)
    useStore.setState({
      settings: normalizeSettings({
        ...DEFAULT_SETTINGS,
        profiles: [profile],
    ...
    vi.mocked(getOpenAICompatibleQueuedImageResult).mockResolvedValueOnce({
      images: ['data:image/png;base64,openai-recovered'],
      actualParams: { n: 1 },
    ...
    expect(getOpenAICompatibleQueuedImageResult).toHaveBeenCalledWith(profile, 'img-task-1', openAITask.params)
    const recovered = useStore.getState().tasks.find((item) => item.id === openAITask.id)
    expect(recovered).toMatchObject({
      status: 'done',
      customRecoverable: false,
      outputImages: expect.any(Array),
      actualParams: { n: 1 },
      revisedPromptByImage: expect.any(Object),
    })
    expect(recovered?.outputImages).toHaveLength(1)
    const outputImage = await getImage(recovered!.outputImages[0])
    expect(outputImage?.dataUrl).toBe('data:image/png;base64,openai-recovered')
  })
})
