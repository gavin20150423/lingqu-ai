...
const RAW_DEFAULT_API_MODEL = readRuntimeEnv(import.meta.env.VITE_DEFAULT_API_MODEL)
const RAW_DEFAULT_API_MODE = readRuntimeEnv(import.meta.env.VITE_DEFAULT_API_MODE)
...
export const DEFAULT_API_MODE: ApiMode = RAW_DEFAULT_API_MODE === 'responses' ? 'responses' : 'images'
export const DEFAULT_IMAGES_MODEL = RAW_DEFAULT_API_MODEL || 'gpt-image-2'
...
export function normalizeSystemApiSettings(input: Partial<AppSettings> | unknown): AppSettings {
  const normalized = normalizeSettings(input)
  const active = normalized.profiles.find((profile) => profile.id === normalized.activeProfileId) ?? normalized.profiles[0] ?? createDefaultOpenAIProfile()
  const record = input && typeof input === 'object' ? input as Record<string, unknown> : {}
  const apiKey = typeof record.apiKey === 'string' && record.apiKey.trim()
    ? record.apiKey
    : active.apiKey
  const systemProfile = createDefaultOpenAIProfile({
    id: DEFAULT_OPENAI_PROFILE_ID,
    name: '系统配置',
    baseUrl: active.baseUrl || DEFAULT_BASE_URL,
    apiKey,
    model: active.model || DEFAULT_IMAGES_MODEL,
    timeout: active.timeout,
  ...
  return normalizeSettings({
    ...
    apiKey: systemProfile.apiKey,
    model: systemProfile.model,
    timeout: systemProfile.timeout,
    apiMode: systemProfile.apiMode,
    codexCli: false,
    apiProxy: systemProfile.apiProxy,
    streamImages: systemProfile.streamImages,
    streamPartialImages: systemProfile.streamPartialImages,
    customProviders: [],
...
export function isSystemApiSettings(input: Partial<AppSettings> | unknown): boolean {
  const normalized = normalizeSettings(input)
  const active = normalized.profiles.find((profile) => profile.id === normalized.activeProfileId) ?? normalized.profiles[0]

  return normalized.customProviders.length === 0 &&
    normalized.profiles.length === 1 &&
    normalized.activeProfileId === DEFAULT_OPENAI_PROFILE_ID &&
    active?.id === DEFAULT_OPENAI_PROFILE_ID &&
    active.name === '系统配置' &&
    active.provider === 'openai' &&
    Boolean(active.baseUrl) &&
    Boolean(active.model) &&
    (active.apiMode === 'images' || active.apiMode === 'responses') &&
    active.codexCli === false &&
    typeof active.apiProxy === 'boolean' &&
    active.streamImages === false &&
    active.streamPartialImages === DEFAULT_STREAM_PARTIAL_IMAGES
}
