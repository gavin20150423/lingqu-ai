...
describe('callImageApi', () => {
  ...
  it('retries image edits as JSON when multipart returns a gateway error', async () => {
    ...
    const result = await callImageApi({
      ...
      prompt: 'round the corners',
      params: { ...DEFAULT_PARAMS },
      inputImageDataUrls: ['data:image/png;base64,aW1n'],
    })

    const apiCalls = fetchMock.mock.calls.filter(([url]) => String(url).includes('/images/edits'))
    expect(apiCalls).toHaveLength(2)
    expect(String(apiCalls[0][0])).toContain('/images/edits')
    expect(String(apiCalls[1][0])).toContain('/images/edits')
    const [, retryInit] = apiCalls[1]
    ...
    expect(JSON.parse(String((retryInit as RequestInit).body))).toMatchObject({
      model: 'gpt-image-2',
      prompt: 'round the corners',
      images: [{ image_url: 'data:image/png;base64,aW1n' }],
    })
  ...
  it('falls back to Responses image_generation when image edits keep returning gateway errors', async () => {
    const fetchMock = vi.spyOn(globalThis, 'fetch')
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify({
        error: { message: 'json bad gateway' },
      }), {
        status: 502,
        headers: { 'Content-Type': 'application/json' },
      }))
    ...
    const apiCalls = fetchMock.mock.calls.filter(([url]) => !String(url).startsWith('data:'))
    expect(apiCalls.map(([url]) => String(url))).toEqual([
      expect.stringContaining('/v1/images/edits'),
      expect.stringContaining('/v1/images/edits'),
      expect.stringContaining('/v1/responses'),
    ])

    const responsesBody = JSON.parse(String((apiCalls[2][1] as RequestInit).body))
    ...
    expect(responsesBody.input[0].content).toEqual([
      expect.objectContaining({ type: 'input_text', text: expect.stringContaining('round the corners') }),
      { type: 'input_image', image_url: 'data:image/png;base64,aW1n' },
    ])
    expect(result.images).toEqual(['data:image/png;base64,ZmFsbGJhY2s='])
  })

  it('polls OpenAI-compatible async image tasks returned by the system proxy', async () => {
    const fetchMock = vi.spyOn(globalThis, 'fetch')
      .mockResolvedValueOnce(new Response(JSON.stringify({
        data: { task_id: 'img-task-1', status: 'queued' },
      }), {
        status: 202,
        headers: { 'Content-Type': 'application/json' },
      }))
    ...
    expect(onCustomTaskEnqueued).toHaveBeenCalledWith({ taskId: 'img-task-1' })
    expect(fetchMock).toHaveBeenCalledTimes(2)
    expect(String(fetchMock.mock.calls[1][0])).toContain('/images/tasks/img-task-1')
    expect(result.images).toEqual(['data:image/png;base64,YXN5bmM='])
  })
