...
export async function getApiErrorMessage(response: Response): Promise<string> {
  ...
  if (response.status === 524) {
    errorMsg = 'HTTP 524：上游网关等待图片生成结果超时。通常是中转站或 Cloudflare 等网关在约 120 秒内没有收到完整响应。可尝试降低尺寸/质量、开启流式传输并请求中间步骤图像，或改用服务端异步任务队列。'
  } else if (response.status === 504) {
    errorMsg = 'HTTP 504：上游网关超时。通常是图片生成耗时超过反向代理或网关限制。可尝试降低尺寸/质量，或改用服务端异步任务队列。'
  } else if (response.status === 502) {
    errorMsg = 'HTTP 502：上游网关返回错误。通常是中转站到模型服务之间连接失败或上游暂时不可用。可稍后重试，或检查中转站服务状态。'
  }
  try {
