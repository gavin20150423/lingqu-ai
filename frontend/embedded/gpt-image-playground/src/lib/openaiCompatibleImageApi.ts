...
function shouldRetryEditAsJson(status: number): boolean {
  return status === 502 || status === 504 || status === 524
}

function shouldFallbackImagesToResponses(status: number): boolean {
  return status === 502 || status === 504 || status === 524
}
...
function getQueuedApiTaskId(payload: unknown): string | null {
  const value = getByPath(payload, 'data.task_id') ?? getByPath(payload, 'task_id') ?? getByPath(payload, 'id')
  const taskId = typeof value === 'string' ? value.trim() : String(value ?? '').trim()
  return taskId || null
}

function getQueuedApiTaskStatus(payload: unknown): string {
  const value = getByPath(payload, 'data.status') ?? getByPath(payload, 'status')
  return typeof value === 'string' ? value.trim().toLowerCase() : String(value ?? '').trim().toLowerCase()
}

function getQueuedApiTaskError(payload: unknown): string {
  const value =
...
function createResponsesImageTool(
  ...
  if (isImageGenerationModel(profile.model)) {
  ...
  try {
    ...
    if (isEdit) {
      ...
      for (let i = 0; i < imageBlobs.length; i++) {
        ...
        formData.append(imageBlobs.length === 1 ? 'image' : 'image[]', blob, `input-${i + 1}.${ext}`)
      }
      ...
      if (!response.ok && shouldRetryEditAsJson(response.status)) {
        const body: Record<string, unknown> = {
          ...
          output_format: params.output_format,
          moderation: params.moderation,
          images: inputImageDataUrls.map((imageUrl) => ({ image_url: imageUrl })),
        }
        ...
        if (opts.maskDataUrl) {
          body.mask = { image_url: opts.maskDataUrl }
        }
    ...
    if (!response.ok) {
      const imagesErrorStatus = response.status
      const imagesErrorMessage = appendImageEditGatewayHint(await getApiErrorMessage(response), imagesErrorStatus, isEdit)
      if (shouldFallbackImagesToResponses(imagesErrorStatus)) {
        try {
          return await callResponsesImageApiSingle(opts, {
            ...profile,
            apiMode: 'responses',
          })
        } catch (fallbackError) {
          const fallbackMessage = fallbackError instanceof Error ? fallbackError.message : String(fallbackError)
          throw new Error(`${imagesErrorMessage}\n\n已自动改用 /v1/responses + image_generation fallback，但仍失败：${fallbackMessage}`)
        }
      }
      throw new Error(imagesErrorMessage)
    }
    ...
    const taskId = getQueuedApiTaskId(payload)
    if (taskId) {
      opts.onCustomTaskEnqueued?.({ taskId })
      clearTimeout(timeoutId)
      return pollOpenAICompatibleTaskResult(
        profile,
        taskId,
        mime,
...
async function pollOpenAICompatibleTaskResult(
  profile: ApiProfile,
  taskId: string,
  mime: string,
  parseResult: (payload: unknown) => Promise<CallApiResult> | CallApiResult,
  signal?: AbortSignal,
): Promise<CallApiResult> {
  const proxyConfig = readClientDevProxyConfig()
  const useApiProxy = shouldUseApiProxy(profile.apiProxy, proxyConfig)
  ...
  while (true) {
    ...
    try {
      const taskResponse = await fetch(buildApiUrl(profile.baseUrl, buildTaskPath('images/tasks/{task_id}', taskId), proxyConfig, useApiProxy), {
        method: 'GET',
        headers: requestHeaders,
        cache: 'no-store',
        signal,
    ...
    const status = getQueuedApiTaskStatus(taskPayload)
    if (status === 'failed' || status === 'failure' || status === 'cancelled' || status === 'canceled') {
      throw new Error(getQueuedApiTaskError(taskPayload) || '异步任务失败')
    }
    if (status === 'completed' || status === 'success' || status === 'succeeded') {
      const resultPayload = getByPath(taskPayload, 'data.result') ?? getByPath(taskPayload, 'result')
      if (!resultPayload) throw new Error('异步任务完成但未返回图片结果')
      try {
        return await parseResult(resultPayload)
      } catch (err) {
        if (!signal?.aborted && isRecoverablePollingError(err)) continue
        throw err
      }
...
function isResponsesApiResponsePayload(payload: unknown): payload is ResponsesApiResponse {
  return Boolean(payload && typeof payload === 'object' && Array.isArray((payload as { output?: unknown }).output))
}

export async function getOpenAICompatibleQueuedImageResult(
  profile: ApiProfile,
  taskId: string,
  params: TaskParams,
): Promise<CallApiResult> {
  ...
  return pollOpenAICompatibleTaskResult(
    ...
    mime,
    async (resultPayload) => {
      if (isResponsesApiResponsePayload(resultPayload)) {
        const imageResults = parseResponsesImageResults(resultPayload, mime)
        const actualParams = mergeActualParams(imageResults[0]?.actualParams ?? {})
        return {
          images: imageResults.map((result) => result.image),
          actualParams,
...
async function callResponsesImageApiSingle(opts: CallApiOptions, profile: ApiProfile): Promise<CallApiResult> {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(appendResponsesImageGatewayHint(await getApiErrorMessage(response), response.status))
    }
    ...
    if (taskId) {
      ...
      return pollOpenAICompatibleTaskResult(
        ...
        mime,
        (resultPayload) => {
          const imageResults = parseResponsesImageResults(resultPayload as ResponsesApiResponse, mime)
          const actualParams = mergeActualParams(
