...
const COPY_IMPORT_URL_OPTIONS_STORAGE_KEY = 'lingqu-image-playground.copy-import-url-options'
...
export default function SettingsModal() {
  ...
  const [draft, setDraft] = useState<AppSettings>(normalizeSystemApiSettings(settings))
  ...
  useEffect(() => {
    ...
    const displaySettings = normalizedSettings.reuseTaskApiProfileTemporarily && reusedTaskApiProfileId && normalizedSettings.profiles.some((profile) => profile.id === reusedTaskApiProfileId)
      ...
      : normalizedSettings
    const nextDraft = normalizeSystemApiSettings({
      ...displaySettings,
  ...
  const commitSettings = (nextDraft: AppSettings) => {
    ...
    const normalizedDraft = normalizeSystemApiSettings({
      ...nextDraft,
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      try {
        ...
        if (imported) {
          const nextDraft = normalizeSystemApiSettings(useStore.getState().settings)
          setDraft(nextDraft)
          setTimeoutInput(String(getActiveApiProfile(nextDraft).timeout))
          setShowProfileMenu(false)
        }
            ...
            {activeTab === 'general' && (
              <div className="space-y-4">
                ...
                <div className="block">
                  ...
                  <div data-selectable-text className="text-xs text-gray-500 dark:text-gray-500">
                    开启后，画廊模式图像生成完成时，会发送浏览器系统通知。浏览器可能会请求通知权限或默认拒绝，请查看相关提示。
                  </div>
                </div>
              </div>
            )}
