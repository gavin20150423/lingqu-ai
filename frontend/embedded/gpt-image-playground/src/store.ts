...
const customRecoveryInFlightTaskIds = new Set<string>()
...
function mergePersistedState(persistedState: unknown, currentState: AppState): AppState {
  ...
  const appMode = getInitialAppMode()
...
async function completeRecoveredOpenAICompatibleTask(task: TaskRecord, result: Awaited<ReturnType<typeof getOpenAICompatibleQueuedImageResult>>) {
  const latest = useStore.getState().tasks.find((item) => item.id === task.id)
  if (!latest || latest.status === 'done') return
  ...
  updateTaskInStore(task.id, {
    outputImages: outputIds,
    transparentOriginalImages: transparentOriginalImageIds,
    actualParams: result.actualParams ?? firstActualParams(actualParamsList),
    actualParamsByImage: mapActualParamsByImage(outputIds, actualParamsList),
    revisedPromptByImage: revisedPromptByImage && Object.keys(revisedPromptByImage).length > 0 ? revisedPromptByImage : undefined,
    status: 'done',
    error: null,
  ...
  useStore.getState().showToast(`异步任务已恢复，共 ${outputIds.length} 张图片`, 'success')
  if (!isAgentTask(task)) showTaskCompletionNotification('图像生成完成', `异步任务已恢复，共 ${outputIds.length} 张图片。`)
}

async function recoverCustomTask(taskId: string) {
  if (customRecoveryInFlightTaskIds.has(taskId)) return
  customRecoveryInFlightTaskIds.add(taskId)
  const { settings, tasks } = useStore.getState()
  const task = tasks.find((item) => item.id === taskId)
  if (!task || !task.customTaskId || task.status === 'done') {
    customRecoveryInFlightTaskIds.delete(taskId)
    return
  }
  ...
  if ((task.apiProvider ?? 'openai') === 'openai') {
    ...
    try {
      const result = await getOpenAICompatibleQueuedImageResult(profile, task.customTaskId, task.params)
      clearCustomRecoveryTimer(taskId)
      await completeRecoveredOpenAICompatibleTask(task, result)
    } catch (err) {
      clearCustomRecoveryTimer(taskId)
      updateTaskInStore(taskId, {
        status: 'error',
...
export async function exportData(options: ExportOptions = { exportConfig: true, exportTasks: true }) {
  try {
    ...
    a.href = url
    a.download = `lingqu-image-playground-backup_${formatExportFileTime(new Date(exportedAt))}.zip`
    a.click()
