...
type ImageTaskResponse = {
    ...
    status?: string;
    poll_url?: string;
    result?: ImageApiResponse | string | null;
    error?: unknown;
    image_url?: string;
};
type GeminiPart = {
...
// Keep the image-workbench resolution tiers aligned with the existing image
// playground. The upstream gateway bills from the actual dimensions, so a
// quality label alone is not enough to request 4K output.
const IMAGE_TIER_SIZES: Record<string, Record<string, string>> = {
    low: {
        "1:1": "1024x1024",
        "3:2": "1536x1024",
        "2:3": "1024x1536",
...
function resolveSize(quality: string | undefined, ratio: string): string {
    ...
    const tierSize = normalizedQuality ? IMAGE_TIER_SIZES[normalizedQuality]?.[ratio] : undefined;
...
function resolveOpenAIRequestSize(model: string, quality: string | undefined, size: string) {
    // The local OpenAI-compatible gateway accepts the same explicit pixel
    // sizes as the image workbench. Do not collapse them to the legacy fixed
    // 1024/1536 canvas sizes: that makes a selected 4K request bill as 2K.
    void model;
...
function imageTaskResult(payload: ImageTaskResponse): ImageApiResponse {
    ...
    if (typeof payload.result === "string") {
        try {
            ...
            if (parsed && typeof parsed === "object") return parsed as ImageApiResponse;
        } catch {
            // The task error is reported below when no image payload can be parsed.
        }
    }
    return {};
}
...
function imageTaskError(payload: ImageTaskResponse) {
    return readApiErrorMessage(payload.error) || apiText("requestFailed");
}

function waitForImageTask(delayMs: number, signal?: AbortSignal) {
...
async function requestAsyncImageTask(
    config: AiConfig,
    path: string,
    ...
    const response = await axios.post<ImageTaskResponse>(aiApiUrl(config, path), body, {
        headers: aiHeaders(config, contentType),
        signal: options?.signal,
    });
    const taskId = String(response.data.task_id || response.data.id || "").trim();
    if (!taskId) {
        // Keep compatibility with an upstream that unexpectedly answers synchronously.
        return parseImagePayload(response.data as ImageApiResponse);
    }

    const deadline = Date.now() + 30 * 60 * 1000;
    for (;;) {
        const task = (await axios.get<ImageTaskResponse>(aiApiUrl(config, `/images/tasks/${encodeURIComponent(taskId)}`), {
            headers: aiHeaders(config),
            signal: options?.signal,
        })).data;
        const status = String(task.status || "").toLowerCase();
        if (["failed", "cancelled", "canceled"].includes(status)) throw new Error(imageTaskError(task));
        if (["completed", "succeeded", "success"].includes(status)) return parseImagePayload(imageTaskResult(task));
        if (Date.now() >= deadline) throw new Error(apiText("requestFailed"));
        await waitForImageTask(3000, options?.signal);
    }
}

async function requestAsyncImageGeneration(config: AiConfig, prompt: string, n: number, quality?: string, size?: string, background?: string, options?: RequestOptions) {
    return requestAsyncImageTask(
        config,
...
async function requestAsyncImageEdit(
    config: AiConfig,
    prompt: string,
    ...
    let files: File[];
    try {
        files = await Promise.all(references.map(async (image) => dataUrlToFile({ ...image, dataUrl: await imageToDataUrl(image) })));
    } catch {
    ...
    return requestAsyncImageTask(config, "/images/edits/async", formData, undefined, options);
}
...
export async function requestGeneration(config: AiConfig, prompt: string, options?: RequestOptions) {
    ...
    const background = normalizeBackground(config.background);
    try {
        if (requestConfig.asyncImageTasks) {
            return await requestAsyncImageGeneration(requestConfig, prompt, n, quality, requestSize, background, options);
        }
...
export async function requestEdit(config: AiConfig, prompt: string, references: ReferenceImage[], mask?: ReferenceImage, options?: RequestOptions) {
    ...
    if (requestConfig.asyncImageTasks) {
        try {
            return await requestAsyncImageEdit(requestConfig, requestPrompt, references, mask, n, quality, requestSize, background, options);
        } catch (error) {
            throw new Error(readAxiosError(error, apiText("requestFailed")));
        }
    }
