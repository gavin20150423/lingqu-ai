...
import { getMediaBlob, normalizeVideoBlob, uploadMediaFile, type UploadedFile } from "@/services/file-storage";
import { imageToDataUrl } from "@/services/image-storage";
import { boolConfig, buildSeedancePromptText, isSeedanceVideoConfig, normalizeSeedanceDuration, normalizeSeedanceRatio, normalizeSeedanceResolution, seedanceVideoReferenceError, SEEDANCE_REFERENCE_LIMITS } from "@/lib/seedance-video";
import { getVideoCapability, videoReferenceLimit, videoSupportsAudio, videoSupportsWatermark } from "@/lib/video-capabilities";
...
type VideoResponse = { id?: string; job_id?: string; status?: string; error?: { message?: string }; url?: string; result_url?: string; video_url?: string; content_url?: string; content?: { video_url?: string; url?: string } | null };
...
type LingquBridge = { videoApiUrl?: string; videoKeyId?: number };
...
/** Read server-side workbench history so records survive browser storage resets. */
export async function listLingquVideoJobs(limit = 100): Promise<VideoWorkbenchJob[]> {
    const bridge = readLingquBridge();
    if (!bridge?.videoApiUrl) return [];
    try {
        const response = await axios.get<{ data?: VideoWorkbenchJob[] }>(`${bridge.videoApiUrl.replace(/\/$/, "")}/jobs`, {
            params: { limit },
            headers: lingquVideoHeaders(),
        });
...
export async function createVideoGenerationTask(config: AiConfig, prompt: string, references: ReferenceImage[] = [], videoReferences: ReferenceVideo[] = [], audioReferences: ReferenceAudio[] = [], options?: RequestOptions): Promise<VideoGenerationTask> {
    ...
    if (isLingquVideoConfig(requestConfig)) {
        return createLingquVideoTask(requestConfig, selectedModel, prompt, references, videoReferences, audioReferences, options);
    }
...
export async function storeGeneratedVideo(result: VideoGenerationResult): Promise<UploadedFile> {
    if (result.blob) {
        ...
        return uploadMediaFile(await normalizeVideoBlob(result.blob, result.mimeType), "video");
    }
    if (result.url) {
        const response = await axios.get<Blob>(result.url, { responseType: "blob" });
        await assertVideoBlob(response.data);
        return uploadMediaFile(await normalizeVideoBlob(response.data, result.mimeType), "video");
    }
...
async function pollOpenAIVideoTask(config: AiConfig, task: VideoGenerationTask, options?: RequestOptions): Promise<VideoGenerationTaskState> {
    try {
        if (isLingquVideoConfig(config)) return pollLingquVideoTask(config, task, options);
        ...
        if (isRetryableVideoQueryError(error)) return { status: "pending" };
        throw new Error(readAxiosError(error, apiText("videoTaskQueryFailed")));
    }
}

async function createLingquVideoTask(config: AiConfig, model: string, prompt: string, references: ReferenceImage[], videoReferences: ReferenceVideo[], audioReferences: ReferenceAudio[], options?: RequestOptions): Promise<VideoGenerationTask> {
    const capability = getVideoCapability(config, model);
    const imageLimit = videoReferenceLimit(capability, "image") ?? 7;
    const videoLimit = videoReferenceLimit(capability, "video") ?? 3;
    const audioLimit = videoReferenceLimit(capability, "audio") ?? 1;
    const imageUrls = await Promise.all(references.slice(0, imageLimit).map((image, index) => uploadLingquImage(config, image, index, options)));
    const videoUrls = await Promise.all(videoReferences.slice(0, videoLimit).map((video, index) => uploadLingquVideo(config, video, index, options)));
    const audioUrls = await Promise.all(audioReferences.slice(0, audioLimit).map((audio, index) => uploadLingquAudio(config, audio, index, options)));
    const hasReferences = imageUrls.length + videoUrls.length + audioUrls.length > 0;
    const body: Record<string, unknown> = {
        model: modelOptionName(model),
        prompt,
        generation_mode: imageUrls.length === 1 && !videoUrls.length && !audioUrls.length ? "first_frame" : hasReferences ? "omni_reference" : "text_to_video",
        resolution: normalizeVideoResolution(config.vquality),
        duration: Number(normalizeVideoSeconds(config.videoSeconds)),
        aspect_ratio: normalizeVideoAspectRatio(config.size),
    };
    if (videoSupportsAudio(capability)) body.audio = boolConfig(config.videoGenerateAudio, false);
    if (videoSupportsWatermark(capability)) body.watermark = boolConfig(config.videoWatermark, false);
    if (imageUrls.length === 1 && !videoUrls.length && !audioUrls.length) {
        body.start_frame_url = imageUrls[0];
    } else if (hasReferences) {
        body.guidances = {
            ...(imageUrls.length ? { image_reference: imageUrls.map((url, index) => ({ image: { url, type: "UPLOADED" }, strength: "MID", order: index + 1 })) } : {}),
            ...(videoUrls.length ? { video_reference_base: videoUrls.map((url) => ({ video: { url, type: "UPLOADED" } })) } : {}),
            ...(audioUrls.length ? { audio_reference: audioUrls.map((url) => ({ audio: { url, type: "UPLOADED" } })) } : {}),
        };
    }
    try {
        const response = await axios.post<VideoResponse>(lingquVideoApiUrl("/jobs"), body, {
            headers: {
                ...lingquVideoHeaders("application/json"),
                Prefer: "respond-async",
                "Idempotency-Key": `canvas-${nanoid()}`,
                "X-Workbench-Request": "infinite-canvas",
            },
            signal: options?.signal,
        });
        const id = response.data.job_id || response.data.id;
...
async function pollLingquVideoTask(config: AiConfig, task: VideoGenerationTask, options?: RequestOptions): Promise<VideoGenerationTaskState> {
    try {
        const response = await axios.get<VideoResponse>(lingquVideoApiUrl(`/jobs/${encodeURIComponent(task.id)}`), { headers: lingquVideoHeaders(), signal: options?.signal });
        const video = response.data;
        if (video.status === "completed") {
            const content = await axios.get<Blob>(lingquVideoApiUrl(`/jobs/${encodeURIComponent(task.id)}/content`), { headers: lingquVideoHeaders(), responseType: "blob", signal: options?.signal });
            await assertVideoBlob(content.data);
            return { status: "completed", result: { blob: content.data } };
        }
        if (video.status === "failed" || video.status === "cancelled" || video.status === "canceled") {
            return { status: "failed", error: readApiErrorMessage(video.error) || apiText("videoGenerationFailed") };
        }
        return { status: "pending" };
    } catch (error) {
        if (isRetryableVideoQueryError(error)) return { status: "pending" };
...
async function videoResultFromUrl(url: string, options?: RequestOptions): Promise<VideoGenerationResult> {
    try {
        ...
        await assertVideoBlob(response.data);
        return { blob: await normalizeVideoBlob(response.data, "video/mp4") };
    } catch (error) {
...
function readLingquBridge(): LingquBridge | null {
    try {
        const key = "lingqu:ai-creation:bridge";
        const raw = window.sessionStorage.getItem(key) || window.localStorage.getItem(key);
        return raw ? (JSON.parse(raw) as LingquBridge) : null;
    } catch {
        return null;
    }
}

function isLingquVideoConfig(_config: AiConfig) {
    return Boolean(readLingquBridge()?.videoApiUrl);
}

function lingquVideoApiUrl(path: string) {
    const baseUrl = readLingquBridge()?.videoApiUrl?.replace(/\/$/, "");
    if (!baseUrl) throw new Error(apiText("baseUrlRequired"));
    return `${baseUrl}${path.startsWith("/") ? path : `/${path}`}`;
}

function lingquVideoHeaders(contentType?: string) {
    const token = window.localStorage.getItem("auth_token");
    const videoKeyId = readLingquBridge()?.videoKeyId;
    return {
        Accept: "application/json",
        ...(token ? { Authorization: `Bearer ${token}` } : {}),
        ...(videoKeyId ? { "X-Video-Key-Id": String(videoKeyId) } : {}),
        ...(contentType ? { "Content-Type": contentType } : {}),
    };
}
...
function isRetryableVideoQueryError(error: unknown) {
...
async function assertVideoBlob(blob: Blob) {
    ...
    const preview = (await blob.slice(0, 256).text()).trimStart();
    if (contentType.includes("html") || contentType.includes("text") || preview.startsWith("<")) {
        throw new Error(apiText("videoDownloadFailed"));
    }
    if (!contentType.includes("json") && !contentType.includes("text") && !preview.startsWith("{")) return;
