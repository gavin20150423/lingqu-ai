...
// The five first-party nodes are part of the canvas experience. Install them on
// first launch so a fresh account can use the same node palette without opening
// the plugin manager manually. A registry outage must never prevent the canvas
// from loading; the manager can still retry installation later.
async function installDefaultOfficialPlugins() {
    let official: Awaited<ReturnType<typeof fetchOfficialPlugins>>;
    try {
        official = await Promise.race([
            fetchOfficialPlugins(),
            new Promise<never>((_, reject) => setTimeout(() => reject(new Error("official plugin registry timeout")), 12_000)),
        ]);
    } catch (error) {
        console.warn("[plugin] Official plugin registry unavailable", error);
        return;
    }

    const installed = new Set(usePluginStore.getState().plugins.map((record) => record.id));
    await Promise.all(
        official
            ...
            .map(async (entry) => {
                try {
                    ...
                    console.warn(`[plugin] Failed to install official plugin: ${entry.id}`, error);
                }
            }),
    );
}
...
export async function ensurePluginsLoaded() {
    ...
    await installDefaultOfficialPlugins();
