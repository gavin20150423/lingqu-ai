...
export default function ImagePage() {
    ...
    const generate = async () => {
        ...
        try {
            ...
            const storedImagesById = new Map(logImages.map((image) => [image.id, image]));
            setResults((current) =>
                current.map((item) =>
                    item.status === "success" && item.image && storedImagesById.has(item.image.id)
                        ? { ...item, image: storedImagesById.get(item.image.id) }
                        : item,
                ),
            );
            saveLog(
    ...
    const addResultToReferences = async (image: GeneratedImage, index: number) => {
        try {
            const stored = await ensureStoredGeneratedImage(image);
    ...
    return (
        <>
            <label className="col-span-2 block min-w-0">
                ...
                <ModelPicker config={config} value={model} onChange={(value) => updateConfig("imageModel", value)} capability="image" grouped fullWidth onMissingConfig={() => openConfigDialog(false)} />
            </label>
...
async function ensureStoredGeneratedImage(image: GeneratedImage) {
    if (image.storageKey) {
        return {
            url: image.dataUrl,
...
async function normalizeLog(log: Partial<GenerationLog>): Promise<GenerationLog> {
    // Older logs can contain a blob URL that belonged to a previous document.
    // Keep durable files and embedded URLs, but do not rehydrate a dead blob URL
    // as a reference that will fail only when the user clicks Generate.
    const references = (await Promise.all((log.references || []).map(normalizeStoredImage))).filter((item): item is ReferenceImage => Boolean(item));
...
async function normalizeStoredImage<T extends { dataUrl?: string; storageKey?: string }>(item: T): Promise<(T & { dataUrl: string }) | null> {
    try {
        const dataUrl = await imageToDataUrl(item);
        return dataUrl.startsWith("data:") ? { ...item, dataUrl } : null;
    } catch {
        return null;
    }
}
