...
import { estimateVideoPrice, getVideoCapability, normalizeRatio, videoReferenceLimit, videoSupportsGuidance, videoSupportsReferenceKind } from "@/lib/video-capabilities";
...
import { createVideoGenerationTask, listLingquVideoJobs, pollVideoGenerationTask, storeGeneratedVideo, type VideoGenerationTask, type VideoWorkbenchJob } from "@/services/api/video";
...
export default function VideoPage() {
    ...
    const videoCapability = getVideoCapability(effectiveConfig, model);
    const legacySeedance = isSeedanceVideoConfig({ ...effectiveConfig, model });
    const supportsReferenceKind = (kind: "image" | "video" | "audio") => videoCapability ? videoSupportsReferenceKind(videoCapability, kind) : legacySeedance;
    const referenceLimits = {
        image: videoReferenceLimit(videoCapability, "image") ?? (legacySeedance ? SEEDANCE_REFERENCE_LIMITS.images : 0),
        video: videoReferenceLimit(videoCapability, "video") ?? (legacySeedance ? SEEDANCE_REFERENCE_LIMITS.videos : 0),
        audio: videoReferenceLimit(videoCapability, "audio") ?? (legacySeedance ? SEEDANCE_REFERENCE_LIMITS.audios : 0),
    };
    const priceEstimate = estimateVideoPrice(effectiveConfig, { hasVideoReference: generationMode === "reference" && videoReferences.length > 0 });
    const canGenerate = Boolean(prompt.trim()) && (!videoCapability || Boolean(priceEstimate));
    ...
    useEffect(() => {
        // A model switch can leave references selected for the previous model.
        // Drop only the kinds that the newly selected model cannot accept, so
        // hidden stale files cannot make a later request fail unexpectedly.
        if (!supportsReferenceKind("image") && references.length) setReferences([]);
        if (!supportsReferenceKind("video") && videoReferences.length) setVideoReferences([]);
        if (!supportsReferenceKind("audio") && audioReferences.length) setAudioReferences([]);
        if (!videoCapability && !legacySeedance && generationMode !== "text") setGenerationMode("text");
        if (videoCapability && !videoSupportsGuidance(videoCapability) && generationMode !== "text") setGenerationMode("text");
    }, [audioReferences.length, generationMode, legacySeedance, references.length, videoCapability, videoReferences.length]);
    ...
    const addReferences = async (files?: FileList | null) => {
        ...
        const imageFiles = selectedFiles.filter((file) => file.type.startsWith("image/") && file.size <= SEEDANCE_REFERENCE_LIMITS.imageMaxBytes).slice(0, Math.max(0, referenceLimits.image - references.length));
        const videoFiles = selectedFiles.filter((file) => SEEDANCE_VIDEO_MIME_TYPES.includes(file.type) && file.size <= SEEDANCE_REFERENCE_LIMITS.videoMaxBytes).slice(0, Math.max(0, referenceLimits.video - videoReferences.length));
        const audioFiles = selectedFiles.filter((file) => isSupportedAudioFile(file) && file.size <= SEEDANCE_REFERENCE_LIMITS.audioMaxBytes).slice(0, Math.max(0, referenceLimits.audio - audioReferences.length));
        ...
        setReferences((value) => [...value, ...nextReferences].slice(0, referenceLimits.image));
        setVideoReferences((value) => [...value, ...nextVideoReferences].slice(0, referenceLimits.video));
        setAudioReferences((value) => [...value, ...nextAudioReferences].slice(0, referenceLimits.audio));
    };
    ...
    const addReferencesFromClipboard = async () => {
        try {
            ...
            const nextReferences = await Promise.all(
                blobs.slice(0, Math.max(0, referenceLimits.image - references.length)).map(async (blob, index) => {
    ...
    const buildRequestSnapshot = () => {
        ...
        const requestReferences: ReferenceImage[] = generationMode === "reference" ? references : [];
        const requestVideoReferences: ReferenceVideo[] = generationMode === "reference" ? videoReferences : [];
        const requestAudioReferences: ReferenceAudio[] = generationMode === "reference" ? audioReferences : [];
        const requestConfig = buildVideoConfig(effectiveConfig, model);
        const capability = getVideoCapability(requestConfig, model);
        if (
            (requestReferences.length > 0 && !supportsReferenceKind("image")) ||
            (requestVideoReferences.length > 0 && !supportsReferenceKind("video")) ||
            (requestAudioReferences.length > 0 && !supportsReferenceKind("audio"))
        ) {
            message.error(t("videoWorkbench.referenceUnsupported"));
            return null;
        }
        if (capability && !estimateVideoPrice(requestConfig, { hasVideoReference: requestVideoReferences.length > 0 })) {
            message.error(t("videoWorkbench.pricingUnavailable"));
            return null;
        }
        ...
        return { text, config: requestConfig, references: [...requestReferences], videoReferences: [...requestVideoReferences], audioReferences: [...requestAudioReferences] };
    };
    ...
    const insertPickedAsset = async (payload: InsertAssetPayload) => {
        if (payload.kind === "text") {
            ...
            if (!supportsReferenceKind("image")) {
                message.warning(t("videoWorkbench.referenceUnsupported"));
                setAssetPickerOpen(false);
                return;
            }
            const stored = await uploadImage(payload);
            setReferences((value) => [...value, { id: nanoid(), name: payload.title, type: stored.mimeType, dataUrl: stored.url, storageKey: stored.storageKey }].slice(0, referenceLimits.image));
            setGenerationMode("reference");
        } else if (payload.kind === "video") {
            if (supportsReferenceKind("video")) {
                setVideoReferences((value) => [...value, { id: nanoid(), name: payload.title, type: "video/mp4", url: payload.url, storageKey: payload.storageKey, width: payload.width, height: payload.height }].slice(0, referenceLimits.video));
                setGenerationMode("reference");
            } else {
                message.warning(t("videoWorkbench.referenceUnsupported"));
            }
    ...
    const refreshLogs = async (resumePending = true, includeRemote = false) => {
        const storedLogs = await readStoredLogs();
        const remoteLogs = includeRemote ? (await listLingquVideoJobs()).map(remoteJobToLog) : [];
        const storedTaskIds = new Set(storedLogs.flatMap((log) => [log.task?.id, log.remoteJobId]).filter((id): id is string => Boolean(id)));
        const nextLogs = [...storedLogs, ...remoteLogs.filter((log) => !storedTaskIds.has(log.remoteJobId || ""))].sort((a, b) => b.createdAt - a.createdAt);
        setLogs(nextLogs);
    ...
    const previewGenerationLog = (log: GenerationLog) => {
        ...
        setGenerationMode(log.references?.length || log.videoReferences?.length || log.audioReferences?.length ? "reference" : "text");
    ...
    return (
        <div className="flex h-full min-w-0 flex-col overflow-hidden bg-stone-50 text-stone-900 dark:bg-stone-950 dark:text-stone-100">
            <main className="grid min-h-0 min-w-0 flex-1 grid-cols-1 gap-3 overflow-x-hidden overflow-y-auto p-3 lg:grid-cols-[300px_minmax(0,1fr)] lg:overflow-hidden xl:grid-cols-[320px_minmax(0,1fr)]">
                ...
                <section className="grid min-w-0 gap-3 lg:min-h-0 lg:overflow-hidden xl:grid-cols-[420px_minmax(0,1fr)]">
                    <div className="thin-scrollbar flex min-w-0 flex-col rounded-lg border border-stone-200 bg-card p-4 shadow-sm dark:border-stone-800 lg:min-h-0 lg:overflow-y-auto">
                        <div className="flex min-w-0 items-start justify-between gap-3">
                            <h1 className="shrink-0 text-xl font-semibold text-stone-950 dark:text-stone-100 sm:text-2xl">{t("videoWorkbench.title")}</h1>
                            <div className="flex shrink-0 gap-2 lg:hidden">
                                <Button icon={<History className="size-4" />} aria-label={t("workbench.logs")} title={t("workbench.logs")} onClick={() => setLogsOpen(true)}>
                                    <span className="hidden sm:inline">{t("workbench.logs")}</span>
                                </Button>
                                <Button icon={<SlidersHorizontal className="size-4" />} aria-label={t("workbench.settings")} title={t("workbench.settings")} onClick={() => setSettingsOpen(true)}>
                        ...
                        <div className="mt-6 space-y-5">
                            <div>
                                <div className="mb-2 flex items-center justify-between gap-3">
                                    ...
                                    <div className="flex shrink-0 gap-2">
                                        <Button size="small" icon={<BookOpen className="size-3.5" />} aria-label={t("workbench.viewPrompts")} title={t("workbench.viewPrompts")} onClick={() => setPromptDialogOpen(true)}>
                                            <span className="hidden sm:inline">{t("workbench.viewPrompts")}</span>
                                        </Button>
                                        <Button size="small" icon={<FolderPlus className="size-3.5" />} aria-label={t("workbench.viewAssets")} title={t("workbench.viewAssets")} onClick={() => setAssetPickerOpen(true)}>
                                            <span className="hidden sm:inline">{t("workbench.viewAssets")}</span>
                                        </Button>
                                    </div>
                                </div>
                            ...
                            <div className="hidden gap-4 sm:grid sm:grid-cols-2">
                                <GenerationSettings config={effectiveConfig} model={model} generationMode={generationMode} onGenerationModeChange={setGenerationMode} updateConfig={updateConfig} openConfigDialog={openConfigDialog} />
                            </div>

                            {generationMode === "reference" ? (
                                <>
                                    {supportsReferenceKind("image") ? (
                                        <div className="min-w-0">
                                            <div className="mb-2 flex items-center justify-between gap-3">
                                                <span className="text-base font-semibold">{t("videoWorkbench.references")}</span>
                                                <div className="flex shrink-0 gap-2">
                                                    <Button size="small" icon={<ClipboardPaste className="size-3.5" />} aria-label={t("workbench.clipboard")} title={t("workbench.clipboard")} onClick={() => void addReferencesFromClipboard()}>
                                                        <span className="hidden sm:inline">{t("workbench.clipboard")}</span>
                                                    </Button>
                                                    <Button size="small" icon={<Upload className="size-3.5" />} aria-label={t("workbench.upload")} title={t("workbench.upload")} onClick={() => fileInputRef.current?.click()}>
                                                        <span className="hidden sm:inline">{t("workbench.upload")}</span>
                                                    </Button>
                                                </div>
                                            </div>
                                            <div
                                            ...
                                            >
                                                ...
                                                {!references.length ? (
                                                    <div className="flex min-w-full items-center justify-center text-sm text-stone-500">{referenceDragTarget === "image" ? t("videoWorkbench.dropReferences") : t("videoWorkbench.noImages")}</div>
                                                ) : null}
                                            </div>
                                        </div>
                                    ) : null}

                                    {supportsReferenceKind("video") ? (
                                        <div className="min-w-0">
                                            <div className="mb-2 flex items-center justify-between gap-3">
                                            ...
                                            >
                                                ...
                                                {!videoReferences.length ? (
                                                    <div className="flex min-w-full items-center justify-center text-sm text-stone-500">{referenceDragTarget === "video" ? t("videoWorkbench.dropReferences") : t("videoWorkbench.noVideos")}</div>
                                                ) : null}
                                            </div>
                                        </div>
                                    ) : null}

                                    {supportsReferenceKind("audio") ? (
                                        <div className="min-w-0">
                                            <div className="mb-2 flex items-center justify-between gap-3">
                                            ...
                                            >
                                                {audioReferences.map((item, index) => (
                                                    <div key={item.id} className="group relative flex h-20 w-48 shrink-0 flex-col justify-center gap-2 rounded-md border border-stone-200 bg-stone-50 px-2 dark:border-stone-800 dark:bg-stone-900">
                                                        ...
                                                        <button
                                                            ...
                                                            aria-label={t("videoWorkbench.removeAudio")}
                                                        >
                                                            <Trash2 className="size-3.5" />
                                                        </button>
                                                    </div>
                                                ...
                                                {!audioReferences.length ? (
                                                    <div className="flex min-w-full items-center justify-center text-center text-sm text-stone-500">{referenceDragTarget === "audio" ? t("videoWorkbench.dropReferences") : t("videoWorkbench.noAudio")}</div>
                                                ) : null}
                                            </div>
                                        </div>
                                    ) : null}
                        ...
                        <div className="mt-auto space-y-2 pt-6">
                            {videoCapability ? (
                                <div className="flex items-center justify-between gap-3 px-1 text-sm">
                                    <span className="text-stone-500 dark:text-stone-400">{t("videoWorkbench.estimatedCost")}</span>
                                    <strong className="text-base text-stone-950 dark:text-stone-100">{priceEstimate ? formatVideoPrice(priceEstimate.currency, priceEstimate.amount) : t("videoWorkbench.pricingUnavailableShort")}</strong>
                                </div>
                            ) : null}
                            <Button type="primary" size="large" block icon={<Sparkles className="size-4" />} loading={running} disabled={!canGenerate || running} onClick={() => void generate()}>
                                {priceEstimate ? t("videoWorkbench.generateWithCost", { amount: formatVideoPrice(priceEstimate.currency, priceEstimate.amount) }) : t("workbench.generate")}
                            </Button>
                            {videoCapability ? <div className="text-center text-xs text-stone-400 dark:text-stone-500">{t("videoWorkbench.finalCostHint")}</div> : null}
                        </div>
                    </div>
...
function GenerationSettings({
    ...
    const { t } = useTranslation();
    const capability = getVideoCapability(config, model);
    const legacySeedance = isSeedanceVideoConfig({ ...config, model });
    const supportsReferenceMode = capability ? videoSupportsGuidance(capability) : legacySeedance;
    return (
        <>
            <label className="col-span-2 block min-w-0">
                <span className="mb-1.5 block text-sm font-semibold sm:mb-2 sm:text-base">{t("workbench.model")}</span>
                <ModelPicker config={config} value={model} onChange={(value) => updateConfig("videoModel", value)} capability="video" grouped stacked fullWidth onMissingConfig={() => openConfigDialog(false)} />
            </label>
            <div className="col-span-2 min-w-0">
                <span className="mb-1.5 block text-sm font-semibold sm:mb-2 sm:text-base">{t("videoWorkbench.generationMode")}</span>
                <div className="grid grid-cols-2 gap-2">
                    <button
                        type="button"
                        className="h-10 min-w-0 rounded-full border px-3 text-sm transition-colors"
                        style={{ borderColor: generationMode === "text" ? theme.node.text : theme.node.stroke, color: theme.node.text }}
                        onClick={() => onGenerationModeChange("text")}
                    >
                        {t("videoWorkbench.generationModes.text")}
                    </button>
                    <button
                        type="button"
                        className="h-10 min-w-0 rounded-full border px-3 text-sm transition-colors disabled:cursor-not-allowed disabled:opacity-35"
                        style={{ borderColor: generationMode === "reference" ? theme.node.text : theme.node.stroke, color: theme.node.text }}
                        disabled={!supportsReferenceMode}
                        title={!supportsReferenceMode ? t("videoWorkbench.referenceUnsupported") : undefined}
                        onClick={() => onGenerationModeChange("reference")}
                    >
                        {t("videoWorkbench.generationModes.reference")}
                    </button>
                </div>
            </div>
        ...
        >
            <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-start gap-2">
                ...
                <div className="grid justify-items-end gap-2">
                    <Tag
                        className={`m-0 flex h-6 items-center rounded-md px-1.5 text-xs leading-none ${log.status === "pending" ? "!border-sky-200 !bg-sky-50 !text-sky-700" : ""}`}
                        color={log.status === "success" ? "blue" : log.status === "pending" ? undefined : "red"}
                    >
...
function remoteJobToLog(job: VideoWorkbenchJob): GenerationLog {
    const id = job.job_id || nanoid();
    const createdAt = Date.parse(job.created_at || job.updated_at || "") || Date.now();
    const status = job.status === "completed" || job.status === "succeeded" ? "success" : job.status === "failed" || job.status === "cancelled" || job.status === "canceled" ? "failed" : "pending";
    const model = job.model || "";
    const seconds = job.duration ? String(job.duration) : "";
    const contentUrl = job.content_url ? new URL(job.content_url, window.location.origin).toString() : "";
    const error = typeof job.error === "string" ? job.error : job.error?.message;
    const video = status === "success" && contentUrl ? { id: `${id}:video`, url: contentUrl, storageKey: "", durationMs: Number(job.duration || 0) * 1000, width: 1280, height: 720, bytes: 0, mimeType: "video/mp4" } : undefined;
    return {
        id: `server:${id}`,
        remoteJobId: id,
        createdAt,
        title: model || i18n.t("workbench.untitled"),
        prompt: "",
        time: new Date(createdAt).toLocaleString(i18n.resolvedLanguage, { hour12: false }),
        model,
        config: { model, videoModel: model, size: job.aspect_ratio || "", vquality: normalizeResolution(job.resolution || ""), videoSeconds: seconds, videoGenerateAudio: "true", videoWatermark: "false" },
        references: [],
        videoReferences: [],
        audioReferences: [],
        durationMs: job.finished_at ? Math.max(0, Date.parse(job.finished_at) - createdAt) : 0,
        size: job.aspect_ratio || "",
        resolution: normalizeResolution(job.resolution || ""),
        seconds,
        status,
        // Server history is read-only here; local pending tasks retain their
        // polling task object, while remote entries must not be polled through
        // the browser's direct OpenAI-compatible endpoint.
        task: undefined,
        video,
        error,
    };
...
function supportsReferenceKindForConfig(config: AiConfig, capability: ReturnType<typeof getVideoCapability>, kind: "image" | "video" | "audio") {
    return capability ? videoSupportsReferenceKind(capability, kind) : isSeedanceVideoConfig(config);
}
...
function buildVideoConfig(config: AiConfig, model: string): AiConfig {
    ...
    const dynamicCapability = getVideoCapability(config, model);
    return {
        ...config,
        model,
        videoModel: model,
        size: seedance ? normalizeSeedanceRatio(config.size) : dynamicCapability ? normalizeRatio(config.size) : normalizeVideoSize(config.size),
...
function normalizeVideoSeconds(value: string) {
    ...
    return String(Math.max(1, Math.min(120, seconds)));
}
...
function formatVideoPrice(currency: string, amount: string) {
    return currency === "USD" ? `US$${amount}` : `${currency} ${amount}`;
}
