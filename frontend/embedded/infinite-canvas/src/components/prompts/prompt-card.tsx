...
export function PromptCard({
    ...
    const coverUrl = promptCoverUrl(item.coverUrl);
    return (
        <Card
            hoverable
...
function promptCoverUrl(value: string) {
    if (!value) return "";
    try {
        const url = new URL(value, window.location.origin);
        // Cached Twitter media URLs expire frequently and otherwise flood the
        // console with 403/404 responses whenever the prompt library mounts.
        return url.hostname === "pbs.twimg.com" ? "" : value;
    } catch {
        return "";
    }
}
