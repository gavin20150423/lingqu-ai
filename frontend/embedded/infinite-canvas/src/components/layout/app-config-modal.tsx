...
export function AppConfigPanel({ showDoneButton = false, initialTab = "channels" }: { showDoneButton?: boolean; initialTab?: ConfigTabKey }) {
    ...
    const hostManagedCredentials = Boolean(config.credentialsManagedByHost);
    ...
    return (
        <>
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-stone-200 pb-3 dark:border-stone-800">
                <div className="text-xs text-stone-500">{hostManagedCredentials ? t("config.credentialsManaged") : t("config.fileSecurity")}</div>
                {!hostManagedCredentials ? (
                        ...
                        children: (
                            <div>
                                <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                                    ...
                                    <Button type="primary" icon={<Plus className="size-4" />} onClick={addChannel} disabled={hostManagedCredentials}>
                                        ...
                                        <div key={channel.id} className="flex items-center justify-between gap-3 rounded-lg border border-stone-200 px-4 py-3 dark:border-stone-800">
                                            <div className="min-w-0">
                                                ...
                                                <div className="mt-1 truncate text-xs text-stone-500">
                                                    {apiFormatLabel(channel.apiFormat, t)} · {t("config.channels.modelCount", { count: channel.models.length })} · {hostManagedCredentials ? t("config.channels.credentialsManaged") : channel.baseUrl || t("config.channels.missingUrl")}
                                                </div>
                                            </div>
                                            <div className="flex shrink-0 gap-2">
                                                ...
                                                {!hostManagedCredentials ? <Button size="small" danger icon={<Trash2 className="size-3.5" />} onClick={() => deleteChannel(channel.id)} /> : null}
                                            </div>
                                        </div>
                                    ))}
                                </div>
            ...
            <ChannelEditorDrawer open={Boolean(editingChannel)} channel={editingChannel} managedCredentials={hostManagedCredentials} onSave={saveChannel} onClose={() => setEditingChannelId("")} />
        </>
    );
}
...
export function AppConfigModal() {
    ...
    const isOpen = useConfigStore((state) => state.isConfigOpen);
    const configTab = useConfigStore((state) => state.configTab);
    const setConfigDialogOpen = useConfigStore((state) => state.setConfigDialogOpen);

    return (
        <Modal open={isOpen} width={980} title={t("config.title")} onCancel={() => setConfigDialogOpen(false)} footer={null} destroyOnHidden>
