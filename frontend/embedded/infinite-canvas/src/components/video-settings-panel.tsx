...
import {
    ...
    videoDefaultAspectRatio,
    videoDefaultDuration,
    videoDefaultResolution,
    videoDurationOptions,
    videoResolutionOptions as capabilityResolutionOptions,
    videoSupportsAudio,
    videoSupportsWatermark,
...
export function VideoSettingsPanel({ config, onConfigChange, theme, showTitle = true, className = "w-[320px] space-y-4 rounded-2xl px-1 py-0.5" }: VideoSettingsPanelProps) {
    ...
    if (getVideoCapability(config)) {
        return <CapabilityVideoSettingsPanel config={config} onConfigChange={onConfigChange} theme={theme} showTitle={showTitle} className={className} />;
    }
...
function CapabilityVideoSettingsPanel({ config, onConfigChange, theme, showTitle, className }: VideoSettingsPanelProps) {
    const { t } = useTranslation();
    const capability = getVideoCapability(config);
    const resolutions = capabilityResolutionOptions(capability);
    const resolution = normalizeResolutionToken(config.vquality || videoDefaultResolution(capability));
    const availableResolution = resolutions.includes(resolution) ? resolution : videoDefaultResolution(capability);
    const ratios = videoAspectRatioOptions(capability, availableResolution);
    const ratio = normalizeRatio(config.size || videoDefaultAspectRatio(capability, availableResolution));
    const availableRatio = ratios.includes(ratio) ? ratio : videoDefaultAspectRatio(capability, availableResolution);
    const durations = videoDurationOptions(capability, availableResolution);
    const numericDuration = Math.floor(Number(config.videoSeconds));
    const duration = durations.includes(numericDuration) ? numericDuration : videoDefaultDuration(capability, availableResolution);
    const supportsAudio = videoSupportsAudio(capability);
    const supportsWatermark = videoSupportsWatermark(capability);

    useEffect(() => {
        if (config.vquality !== availableResolution) onConfigChange("vquality", availableResolution);
        if (normalizeRatio(config.size) !== availableRatio) onConfigChange("size", availableRatio);
        if (String(numericDuration) !== String(duration)) onConfigChange("videoSeconds", String(duration));
        if (!supportsAudio && config.videoGenerateAudio !== "false") onConfigChange("videoGenerateAudio", "false");
        if (!supportsWatermark && config.videoWatermark !== "false") onConfigChange("videoWatermark", "false");
    }, [availableRatio, availableResolution, config.size, config.vquality, config.videoGenerateAudio, config.videoWatermark, duration, numericDuration, onConfigChange, supportsAudio, supportsWatermark]);
    ...
    return (
        <ImageSettingsTheme theme={theme}>
            <div className={className} style={{ color: theme.node.text }} onMouseDown={(event) => event.stopPropagation()}>
                ...
                <SettingGroup title={t("settingsPanels.video.resolution")} color={theme.node.muted}>
                    <div className="grid grid-cols-3 gap-2.5">
                        {resolutions.map((value) => (
                            <OptionPill key={value} selected={availableResolution === value} theme={theme} onClick={() => onConfigChange("vquality", value)}>
                                {value}
                            </OptionPill>
                        ))}
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-2.5">
                        {ratios.map((value) => (
                            <button
                                ...
                                style={{ borderColor: availableRatio === value ? theme.node.text : theme.node.stroke, color: theme.node.text }}
                                onMouseDown={(event) => event.stopPropagation()}
                ...
                {supportsAudio || supportsWatermark ? (
                    <SettingGroup title={t("settingsPanels.video.output")} color={theme.node.muted}>
                        <div className="grid gap-2 rounded-xl border p-2.5" style={{ borderColor: theme.node.stroke }}>
                            {supportsAudio ? (
                                <SwitchRow label={t("settingsPanels.video.generateAudio")} checked={boolConfig(config.videoGenerateAudio, false)} theme={theme} onChange={(checked) => onConfigChange("videoGenerateAudio", String(checked))} />
                            ) : null}
                            {supportsWatermark ? <SwitchRow label={t("settingsPanels.video.watermark")} checked={boolConfig(config.videoWatermark, false)} theme={theme} onChange={(checked) => onConfigChange("videoWatermark", String(checked))} /> : null}
                        </div>
                    </SettingGroup>
                ) : null}
            </div>
...
function ratioLabel(ratio: string) {
    ...
    return i18n.t(`settingsPanels.video.ratios.${labels[ratio] || "adaptive"}`);
}
