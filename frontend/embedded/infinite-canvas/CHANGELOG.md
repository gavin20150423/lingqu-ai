...
+ [新增] 作为灵渠 AI 创作工作区嵌入，自动接入当前登录态、API Key 与视频模型能力。
+ [新增] 视频工作台和画布视频节点接入灵渠异步任务、素材上传、状态查询与成片下载接口。
+ [新增] 提示词来源新增 Freestylefly GPT Image 2 内置来源。

## v0.16.0 - 2026-08-18

+ [同步] 画布基线升级至上游 Infinite Canvas v0.16.0。
+ [兼容] 保留灵渠宿主注入、视频任务、渠道能力和本地用户界面裁剪。
