    ...
    const listLocalPlugins = () => {
        try {
            return readdirSync(pluginsDir)
                ...
                .sort()
                .map((file) => `${process.env.VITE_BASE || "/ai-creation/"}plugins/${file}`);
        } catch {
            return [];
        }
    };
...
export default defineConfig({
    base: process.env.VITE_BASE || "/ai-creation/",
    ...
    server: {
        ...
        proxy: {
            ...
            "/v1": { target: process.env.VITE_DEV_PROXY_TARGET || "http://127.0.0.1:8080", changeOrigin: true },
        },
    },
});
