...
async function submitCheckout() {
  ...
  try {
    ...
    await launchPayment(storeOrder)
  } catch (err: unknown) {
    appStore.showError(extractI18nErrorMessage(err, t, 'store.errors', t('common.error')))
  } finally {
...
async function launchPayment(storeOrder: StoreOrder) {
  ...
  if (decision.kind === 'wechat_jsapi' && decision.jsapi) {
    try {
      const result = await invokeWechatJsapiPayment(decision.jsapi as Record<string, unknown>)
  ...
  if (decision.kind === 'stripe_popup' || decision.kind === 'redirect_waiting') {
    ...
    if (url) {
      if (decision.kind === 'redirect_waiting' && isMobileDevice()) {
