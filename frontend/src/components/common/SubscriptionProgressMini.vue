        ...
        <div class="max-h-64 overflow-y-auto">
          ...
          >
            ...
            <div class="space-y-1.5">
              ...
              <template v-else>
                <div v-if="getLimit(subscription, 'daily')" class="flex items-center gap-2">
                  ...
                  <span class="w-24 flex-shrink-0 text-right text-[10px] text-gray-500">
                    {{
                      formatUsage(subscription.daily_usage_usd, getLimit(subscription, 'daily'))
                    }}
                  </span>
                </div>

                <div v-if="getLimit(subscription, 'weekly')" class="flex items-center gap-2">
                  <span class="w-8 flex-shrink-0 text-[10px] text-gray-500">{{
                  ...
                  <span class="w-24 flex-shrink-0 text-right text-[10px] text-gray-500">
                    {{
                      formatUsage(subscription.weekly_usage_usd, getLimit(subscription, 'weekly'))
                    }}
                  </span>
                </div>

                <div v-if="getLimit(subscription, 'monthly')" class="flex items-center gap-2">
                  <span class="w-8 flex-shrink-0 text-[10px] text-gray-500">{{
...
function getMaxUsagePercentage(sub: UserSubscription): number {
  ...
  const dailyLimit = getLimit(sub, 'daily')
  const weeklyLimit = getLimit(sub, 'weekly')
  const monthlyLimit = getLimit(sub, 'monthly')
  if (dailyLimit) {
...
function isUnlimited(sub: UserSubscription): boolean {
  return !getLimit(sub, 'daily') && !getLimit(sub, 'weekly') && !getLimit(sub, 'monthly')
}

type QuotaWindow = 'daily' | 'weekly' | 'monthly'

function getLimit(sub: UserSubscription, window: QuotaWindow): number | null | undefined {
