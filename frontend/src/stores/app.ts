...
export const useAppStore = defineStore('app', () => {
  ...
  let rawPublicSettings: PublicSettings | null = null
  ...
  const updateEnabled = ref<boolean>(true)
  ...
  async function fetchVersion(force = false): Promise<VersionInfo | null> {
    ...
    try {
      ...
      updateEnabled.value = data.update_enabled ?? true
  ...
  function fetchPublicSettings(force = false): Promise<PublicSettings | null> {
    ...
    if (publicSettingsLoaded.value && !force) {
      if (rawPublicSettings) {
        return Promise.resolve({ ...rawPublicSettings })
      }
      return Promise.resolve({
        ...
        site_subtitle: DEFAULT_SITE_TAGLINE,
