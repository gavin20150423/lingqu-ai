export default {
    ...
    backup: {
      ...
      imageStorage: {
        title: '异步生图存储',
        description: '异步生图始终可用。默认将结果保存在服务器持久化目录；开启 S3 后改用对象存储，保存后立即生效，无需重启。',
        enabled: '使用 S3 对象存储',
        localFallback: '当前使用服务器本地持久化存储，图片通过本平台地址访问。',
    ...
    groups: {
      ...
      form: {
        ...
        autoAssignByRate: '按账号倍率自动归集',
        autoAssignByRateHint: '自动维护当前平台中符合倍率上限的账号；手动绑定不受影响',
        autoAssignMaxRate: '账号倍率上限',
        autoAssignMaxRateHint: '包含倍率小于或等于此值的同平台账号，账号倍率变化后会自动移入或移出',
        autoAssignMaxRateRequired: '启用自动归集时，请填写大于或等于 0 的账号倍率上限',
        rpmLimit: '每分钟请求数 (RPM)',
        rpmLimitPlaceholder: '0 表示不限制',
