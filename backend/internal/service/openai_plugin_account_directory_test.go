...
func TestAccountReadableSnapshot_DenylistTripwire(t *testing.T) {
	...
	safeToExpose := map[string]struct{}{
		...
		// 本仓（lingqu fork）在上游模型之上追加的账号共享/会员归属与计费分层字段：
		// 全部为 id、枚举状态或数值阈值，均不含凭据，按 denylist 契约归入可读元数据。
		"AccountLevel": {}, "OwnerUserID": {}, "ShareMode": {}, "ShareStatus": {},
		"SharePolicyID": {}, "AccountShareModeListingID": {},
		"LoadFactorPaidCeiling": {}, "ErrorSince": {},
	}
