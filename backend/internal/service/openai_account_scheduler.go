...
func (s *OpenAIGatewayService) ReportOpenAIAccountScheduleResult(account any, model string, success bool, firstTokenMs *int, observedErr ...error) bool {
	...
	healthTripped := false
	if accountObj != nil && s != nil && s.rateLimitService != nil {
		if success {
			s.rateLimitService.ObserveOpenAIAPIKeyHealthSuccess(context.Background(), accountObj)
		} else if len(observedErr) > 0 && observedErr[0] != nil {
			healthTripped = s.rateLimitService.ObserveOpenAIAPIKeyHealthFailure(context.Background(), accountObj, observedErr[0])
		}
	}
	if success {
