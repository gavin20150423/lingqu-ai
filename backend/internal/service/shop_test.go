...
func TestShopBalancePaymentDeliversLoadFactorCredits(t *testing.T) {
	ctx := context.Background()
	client := newPaymentConfigServiceTestClient(t)
	svc := NewShopService(client, nil, nil, nil)
	user := createShopTestUser(t, ctx, client, "load-factor-credits@example.com")
	user, err := client.User.UpdateOneID(user.ID).SetBalance(100).Save(ctx)
	require.NoError(t, err)
	product, err := client.ShopProduct.Create().
		SetName("Load factor credits").
		SetPrice(2).
		SetEnabled(true).
		SetMinPurchase(1).
		SetMaxPurchase(10).
		SetAutoDelivery(true).
		SetProductType(ShopProductTypeLoadFactorCredits).
		SetAllowBalancePayment(true).
		SetAllowPlatformPayment(false).
		SetLoadFactorCreditsPerUnit(4).
		Save(ctx)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Equal(t, ShopOrderStatusCompleted, order.Status)
	require.Equal(t, 12, order.LoadFactorCreditsAwarded)

	user, err = client.User.Get(ctx, user.ID)
	require.NoError(t, err)
	require.Equal(t, 94.0, normalizeShopAmount(user.Balance))
	require.Equal(t, 12, user.LoadFactorCreditsBalance)
}
