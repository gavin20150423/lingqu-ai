...
// StableGrokAudioBillingRequestID provides a deterministic deduplication key.
func StableGrokAudioBillingRequestID(upstreamRequestID string) string {
	requestID := strings.TrimSpace(upstreamRequestID)
	if strings.HasPrefix(requestID, "grok_audio:") {
		return requestID
	}
	if requestID == "" {
		requestID = generateRequestID()
	}
	return "grok_audio:" + requestID
}

// StableGrokRealtimeBillingRequestID provides a deterministic session key.
...
func resolveUsageBillingRequestID(ctx context.Context, upstreamRequestID string) string {
	// Durable money-event IDs must win over reusable client request IDs.
...
func (s *GatewayService) calculateRecordUsageCost(
	...
	if result.AudioUsage != nil {
		if resolved := s.resolveChannelPricing(ctx, billingModel, apiKey); resolved != nil && resolved.Mode == BillingModePerRequest {
			gid := apiKey.Group.ID
			cost, err := s.billingService.CalculateCostUnified(CostInput{Ctx: ctx, Model: billingModel, GroupID: &gid, Group: apiKey.Group, UsageUnits: result.AudioUsage.DurationOrUnits, SizeTier: result.AudioUsage.Mode, RateMultiplier: multiplier, Resolver: s.resolver, Resolved: resolved})
			if err == nil {
				return cost
			}
		}
	...
	// Token 计费；搜索请求的按次附加费与 token 费用叠加。
