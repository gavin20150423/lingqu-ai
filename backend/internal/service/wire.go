...
// ProvideUpdateService creates UpdateService with BuildInfo and update config.
...
// ProvideCommunityBillingService starts idle-expiry and billing-window settlement.
func ProvideCommunityBillingService(accountRepo AccountRepository) *CommunityBillingService {
	repo, _ := accountRepo.(CommunityBillingSweeper)
	svc := NewCommunityBillingService(repo, time.Minute)
	svc.Start()
	return svc
}
...
func ProvideImageStorageSettingService(
	...
	if cfg.ImageStorage.Enabled && !cfg.ImageStorage.Active() {
		...
		logger.L().Warn("image_storage S3 is enabled but not fully configured; async image tasks will use local storage",
...
// 存储选择由 settings 服务在运行时解析，因此后台改开关后无需重启即可生效。
...
var ProviderSet = wire.NewSet(
	...
	ProvideCommunityBillingService,
