...
// CyberSessionBlockKey keeps compatibility with older handler call sites.
...
func (s *OpenAIGatewayService) MarkCyberSessionBlocked(ctx context.Context, scopeKey string, keySets ...[]string) {
	var keys []string
	if len(keySets) > 0 {
		keys = keySets[0]
	} else if strings.TrimSpace(scopeKey) != "" {
...
// IsCyberSessionBlocked performs a fail-open single-key lookup for legacy
// handlers that already derived the exact block key.
