...
func (s *ShopService) createPlatformPaymentOrder(ctx context.Context, req ShopCreateOrderRequest) (*ShopOrderDTO, error) {
	...
	if err := tx.Commit(); err != nil {
		return nil, fmt.Errorf("commit shop order: %w", err)
	}
	order.Unwrap()
	...
	if err != nil {
		_, _ = s.entClient.ShopOrder.Update().
			Where(shoporder.IDEQ(order.ID), shoporder.StatusEQ(ShopOrderStatusPending)).
			SetStatus(ShopOrderStatusFailed).
			SetFailedReason(err.Error()).
			Save(ctx)
		return nil, err
	}
	linkTx, err := s.entClient.Tx(ctx)
	if err != nil {
		return nil, fmt.Errorf("begin shop order link transaction: %w", err)
	}
	defer func() { _ = linkTx.Rollback() }()
	order, err = linkTx.ShopOrder.UpdateOneID(order.ID).
		SetPaymentOrderID(paymentResp.OrderID).
		Save(ctx)
	if err != nil {
		return nil, fmt.Errorf("link shop payment order: %w", err)
	}
	if product.ProductType == ShopProductTypeCardKey {
		if _, err := s.reserveCardsForOrderInTx(ctx, linkTx, order, paymentResp.ExpiresAt); err != nil {
			return nil, err
		}
	}
