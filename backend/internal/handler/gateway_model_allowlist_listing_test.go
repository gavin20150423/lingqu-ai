...
func TestGeminiV1BetaListModels_FiltersFallbackByAllowlist(t *testing.T) {
	...
	// lingqu 定制：白名单开启时 customGeminiModelsList 直接发布「白名单展开后的目录」，
	// 通配 gemini-3-* 只在内置目录 gemini.DefaultModels() 上展开，故命中
	// gemini-3-flash-preview（内置目录中不含裸 gemini-3-flash，后者来自 antigravity 列表）。
	require.Contains(t, names, "models/gemini-3-flash-preview")
