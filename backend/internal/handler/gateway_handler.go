	...
	if apiKey.Group != nil && apiKey.Group.IsSubscriptionType() {
		...
		if ok {
			...
			resp["subscription"] = gin.H{
				...
				"daily_limit_usd":     subscription.EffectiveDailyLimitUSD(apiKey.Group),
				"weekly_limit_usd":    subscription.EffectiveWeeklyLimitUSD(apiKey.Group),
				"monthly_limit_usd":   subscription.EffectiveMonthlyLimitUSD(apiKey.Group),
...
func (h *GatewayHandler) handleFailoverExhausted(c *gin.Context, failoverErr *service.UpstreamFailoverError, platform string, streamStarted bool) {
	...
	if !streamStarted && h.cfg != nil && h.cfg.Gateway.PassthroughUpstreamErrors &&
		writeRawUpstreamError(c, statusCode, failoverErr.ResponseHeaders, responseBody) {
...
func billingErrorDetails(err error) (status int, code, message string, retryAfter int) {
	...
	if errors.Is(err, service.ErrSubscriptionEntitlementExhausted) {
		msg := pkgerrors.Message(err)
		return http.StatusTooManyRequests, "subscription_entitlement_exhausted", msg, 0
	}
	msg := pkgerrors.Message(err)
	if msg == "" {
		logger.L().With(
