...
func classifyOpsPhase(errType, message, code string) string {
	...
	case "rate_limit_error":
		...
		return "upstream"
	case "invalid_request_error", "permission_error", "forbidden_error", "content_policy_error", "not_found_error", "model_not_found":
		return "request"
...
func classifyOpsSeverity(errType string, status int) string {
	switch errType {
	case "invalid_request_error", "authentication_error", "permission_error", "content_policy_error", "forbidden_error", "not_found_error", "model_not_found", "billing_error", "subscription_error":
		return "P3"
	}
	if status >= 500 {
		return "P1"
...
func classifyOpsErrorLog(c *gin.Context, errType, message, code string, status int) (phase string, isBusinessLimited bool, errorOwner string, errorSource string) {
	...
	requestScopedClientDenial := errType == "content_policy_error"
	accountAuthFailure := hasOpsAccountAuthFailure(c)
	if localModelConfiguration {
		phase = "routing"
	} else if accountAuthFailure && !routingCapacityLimited {
		phase = "account_auth"
	} else if upstreamError && !routingCapacityLimited && !requestScopedClientDenial {
		phase = "upstream"
	}
	...
	isBusinessLimited = requestScopedClientDenial || localModelConfiguration || routingCapacityLimited || (clientBusinessLimited && !effectiveUpstreamError) || localBusinessLimited
