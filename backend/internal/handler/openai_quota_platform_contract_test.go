...
func TestSubPilotSelectedOpenAIUsageInputsCarryLeaseAndSession(t *testing.T) {
	files := []string{
		"openai_gateway_handler.go",
		"openai_chat_completions.go",
		"openai_embeddings.go",
		...
		t.Run(name, func(t *testing.T) {
			...
			ast.Inspect(file, func(node ast.Node) bool {
				...
				for field := range missing {
					if !compositeLiteralHasKey(literal, field) {
						missing[field] = append(missing[field], fset.Position(literal.Lbrace))
					}
				}
				return true
			})

			require.Empty(t, missing["SubPilotLeaseID"], "SubPilot-selected OpenAI usage must release the original lease")
			require.Empty(t, missing["SubPilotSessionKey"], "SubPilot-selected OpenAI usage must preserve session affinity")
		})
	}
}
