...
func (Group) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Float("rate_multiplier").
			...
			Default(1.0),
		// When enabled, this group continuously contains accounts on the same
		// platform whose account-level billing multiplier is at or below the cap.
		field.Bool("auto_assign_accounts_by_rate").
			Default(false),
		field.Float("auto_assign_max_rate").
			Optional().
			Nillable().
			SchemaType(map[string]string{dialect.Postgres: "decimal(10,4)"}),
